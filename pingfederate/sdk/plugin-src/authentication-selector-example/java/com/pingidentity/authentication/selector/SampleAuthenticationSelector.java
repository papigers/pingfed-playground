 ...
 */
public class SampleAuthenticationSelector implements AuthenticationSelector
{
    // HTML field names
            ...
            Map<String, Object> extraParameters, String resumePath)
    {
        ...
        // use the cookie value to build the returning AuthenticationSelectorContex
        String cookieName = getCookieName();
        String domain = getCookieValue(cookieName, req);
        if (domain == null)
        // There is no cookie with a domain name, check the request
        {
            String inputEmailOrDomain = req.getParameter(EMAIL_INPUT_FIELD_NAME);
            ...
            {
                boolean emailDomainAddressPostedBlank = false;

                String cancelFieldValue = req.getParameter(PF_CANCEL);
                if (StringUtils.isNotBlank(cancelFieldValue))
                // The cancel button was clicked
                {
                    return context;
                }
                else if (inputEmailOrDomain != null && inputEmailOrDomain.isEmpty())
                {
                    emailDomainAddressPostedBlank = true;
                }

                renderForm(req, resp, resumePath, emailDomainAddressPostedBlank );

                log.debug("Rendering the form to allow the user to provide a email address or a domain name.");
                return null;
            }
        }
